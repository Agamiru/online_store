...
table = bs(html_data, features="html.parser")("tr")
table_data = [[cell.text for cell in row("td")] for row in table]
print(f"len_table_data: {len(table_data)}\n\n")
# values = [[cell.text for cell in row("span")] for row in table]
...
json_data = json.dumps(new_list, indent=2, ensure_ascii=False)
