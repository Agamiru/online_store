...
def string_list_to_list(string_list: str) -> list:
    """
    Convert string lists to real lists
    """
    if not isinstance(string_list, str):
        raise TypeError("Function only accepts strings")
    string_list = string_list.replace("[", "").replace("]", "").strip()
    string_list = [word.replace('"', "").strip() for word in string_list.split(",")]
...
#     tabul = BhphotovideoTableConverter(html_data_2)
#     tabul.convert()
#     json_table = tabul.to_json()
